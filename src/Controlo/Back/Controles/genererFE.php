...
elseif($nbSalles != $nbEnseignantsSurveillants){
    ...
    function annee_universitaire($date) {
        ...
        // Déterminer l'année universitaire correspondante
        if ($date->format('n') < 9) {
          // L'année universitaire commence en septembre de l'année précédente
          $annee_universitaire = ($annee - 1) . '-' . $annee;
        } else {
          // L'année universitaire commence en septembre de l'année en cours
          $annee_universitaire = $annee . '-' . ($annee + 1);
        }
        
        // Retourner l'année universitaire
        return $annee_universitaire;
    }

    $dateControle = $unControle->getDate();

    // Transformer la date actuellement en DD/MM/YYYY en YYYY-MM-DD
    $dateControle = str_replace('/', '-', $dateControle);
    $dateControle = date('Y-m-d', strtotime($dateControle));


    // Transformer la date du contrôle en année universitaire
    $anneeUniversitaire = annee_universitaire($dateControle);

    if(isset($_POST["anneeUniversitaire"])){
        $anneeUniversitaire = htmlspecialchars($_POST["anneeUniversitaire"]);
        genererPDFFE($unControle, $anneeUniversitaire);
    ...
    // Formulaire pour générer les feuilles d'émargement
    echo "
        <div class='container'>
            <div class=\"col-3\"></div>
            <div class=\"col-6 m-auto text-center\">

            <form action='".PAGE_GENERER_FE_PATH."' method='post'>
                <input type='hidden' name='idControle' value='$idControle'>
                <div class='form-group row'>
                    <label for='anneeUniversitaire' class='col-4 col-form-label'>Année universitaire à afficher</label>
                    <input class='col-8' type='text' class='form-control' id='anneeUniversitaire' name='anneeUniversitaire' value='$anneeUniversitaire'>
                </div>
                <button type='submit' class='btn btn-primary'>
                    <i class='fas fa-download'></i> Générer les feuilles d'émargement
                </button>
            </form>
